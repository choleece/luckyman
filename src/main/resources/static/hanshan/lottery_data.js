...
//        $('#lottery-wrap').css('top', -stop_top);
    ...
    var sure_index_arr = [sure_index];
    // 如果不是特等奖，其他的三类奖项均为一次开奖5人, 则上下取其二
    if (award !== 1) {
        sure_index_arr = [sure_index - 2, sure_index - 1, sure_index, sure_index + 1, sure_index + 2];
    }
...
function drawAward(award, arr, pic_format) {
    ...
    back_img.onload = function() {
        ...
        for (var i = 0; i < arr.length; i++) {
            var item = arr[i];
            var height = item.namezh.length >= 3 ? 300 : 280;
            context.fillText(item.namezh, height, 930 + 90 * i);
        }
    };
}
